    ...
    private static Map<String, String> splitQuery(String str) {
        ...
        while (true) {
            ...
            if (str.charAt(i) == '&') {
                ...
                if (indexOf < 0) {
                    if (indexOf2 < 0) {
                        str2 = Uri.decode(str.substring(i).replace('+', TokenParser.SP));
                    } else {
                        String decode = Uri.decode(str.substring(i, indexOf2).replace('+', TokenParser.SP));
                        str3 = Uri.decode(str.substring(indexOf2 + 1).replace('+', TokenParser.SP));
                        str2 = decode;
                    }
                    ...
                    if (indexOf2 < 0 || indexOf2 > indexOf) {
                        String decode2 = Uri.decode(str.substring(i, indexOf).replace('+', TokenParser.SP));
                        if (!hashMap.containsKey(decode2)) {
                            hashMap.put(decode2, "");
                        }
                    } else {
                        String decode3 = Uri.decode(str.substring(i, indexOf2).replace('+', TokenParser.SP));
                        String decode4 = Uri.decode(str.substring(indexOf2 + 1, indexOf).replace('+', TokenParser.SP));
